"""Temporal Workflow for Policy Deployment & Execution in PostgreSQL."""
...
async def compile_policy_activity(params: dict) -> dict:
    """Activity that fetches raw policy from PostgreSQL."""
    version_id = params["version_id"]
    async with AsyncSessionLocal() as db:
        raw = await fetch_policy_raw_payload(version_id, db)
        return {
            "raw_payload": raw,
            "snowflake_sql": "",
            "redshift_sql": "",
...
async def deploy_to_platform_activity(params: dict) -> dict:
    """Activity that sends compiled deploy requests to platform connectors."""
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            if resp.status_code == 200 and result.get("success"):
                return {
                    ...
                    or f"Successfully applied native {platform_code} DDL",
                }
            else:
                err_msg = (
                    result.get("error")
                    or result.get("message")
                    or f"Connector returned HTTP {resp.status_code}"
                )
...
class PolicyDeploymentWorkflow:
    ...
    async def run(self, payload: dict) -> dict:
        ...
        for target in targets:
            ...
            sql_ddl = (
                compile_res["snowflake_sql"]
                if platform_code == "SNOWFLAKE"
                else compile_res["redshift_sql"]
            )
        ...
        if failed_errors:
            raise RuntimeError(
                f"Workflow execution failed after 3 retry attempts: {'; '.join(failed_errors)}"
            )
