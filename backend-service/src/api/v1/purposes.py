...
async def list_purposes(db: AsyncSession = Depends(get_db)):
    ...
    rows = (await db.execute(text("""
            SELECT
                ...
                p.purpose_name,
                p.description,
                p.compliance_mandate AS regulatory_mandate,
                'RESTRICTED' AS max_sensitivity,
                p.retention_period_days AS retention_days,
                p.is_active,
...
async def create_purpose(
    ...
    res = await db.execute(
        text("""
            INSERT INTO purposes (purpose_code, purpose_name, description, compliance_mandate, retention_period_days)
            VALUES (:c, :n, :d, :m, :r)
            RETURNING purpose_id, purpose_code, purpose_name, description, compliance_mandate AS regulatory_mandate, 'RESTRICTED' AS max_sensitivity, retention_period_days AS retention_days, is_active
        """),
        {
