...
PLATFORM_DRIVERS = [
    {
        ...
        "fields": ["account_identifier", "warehouse", "default_database", "role", "db_user", "db_password"],
    },
    {
...
async def get_platform(platform_id: int, db: AsyncSession = Depends(get_db)):
    row = (await db.execute(text("SELECT * FROM metadata_platforms WHERE platform_id = :p AND is_active = TRUE"), {"p": platform_id})).mappings().first()
    if not row:
...
async def create_platform(body: PlatformCreate, db: AsyncSession = Depends(get_db)):
    ...
    res = await db.execute(
        text("""
            INSERT INTO metadata_platforms (platform_code, platform_name, platform_version, connection_alias, is_active)
            VALUES (:c, :n, :v, :a, TRUE)
            ON CONFLICT (platform_code) DO UPDATE SET platform_name=EXCLUDED.platform_name, connection_alias=EXCLUDED.connection_alias, is_active=TRUE
            RETURNING platform_id, platform_code, platform_name, connection_alias, is_active, created_at
        """),
        {"c": body.platform_code, "n": body.platform_name, "v": body.platform_version, "a": alias}
    )
...
async def update_platform(platform_id: int, body: PlatformUpdate, db: AsyncSession = Depends(get_db)):
    res = await db.execute(
        text("""
            ...
            RETURNING platform_id, platform_code, platform_name, connection_alias, is_active
        """),
        {"p": platform_id, "n": body.platform_name, "a": body.connection_alias}
    )
    row = res.mappings().first()
    await db.commit()
    if not row:
...
from src.services.platform_tester import test_db_connection

@router.post("/platforms/test-connection")
async def test_platform_connection(body: TestConnectionRequest):
    """
    Stateless connection test: Attempt live database authentication against the target platform
    using native driver connectors without modifying or saving data to the PostgreSQL database.
    """
    return test_db_connection(body.platform_code, body.model_dump())

@router.delete("/platforms/{platform_id}")
async def delete_platform(platform_id: int, db: AsyncSession = Depends(get_db)):
    await db.execute(text("UPDATE metadata_platforms SET is_active = FALSE WHERE platform_id = :p"), {"p": platform_id})
    await db.commit()
...
async def get_tags_tree(db: AsyncSession = Depends(get_db)):
    """Return Immuta hierarchical tree of tags."""
    query = text("""
        SELECT 
            t.tag_id,
            t.tag_name,
...
async def create_tag(req: TagCreateRequest, db: AsyncSession = Depends(get_db)):
    """Create a new custom Immuta hierarchical tag."""
    parent_path = None
...
from src.services.tag_scanner import run_sensitive_data_discovery

@router.post("/tags/discover")
async def trigger_sensitive_data_discovery(db: AsyncSession = Depends(get_db)):
    """Trigger the Immuta Sensitive Data Discovery scanner across data catalog assets."""
...
async def sync_platform_tags(db: AsyncSession = Depends(get_db)):
    ...
    platforms = (await db.execute(text("SELECT platform_id, platform_code, platform_name FROM metadata_platforms WHERE is_active = TRUE"))).mappings().all()
    synced = []
    for p in platforms:
...
async def assign_tag(req: TagAssignRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not req.table_id and not req.column_id:
        raise HTTPException(status_code=400, detail="Either table_id or column_id must be specified")
...
async def unassign_tag(assignment_id: int, db: AsyncSession = Depends(get_db)):
    await db.execute(text("DELETE FROM metadata_tag_assignments WHERE assignment_id = :aid"), {"aid": assignment_id})
    await db.commit()
...
async def get_dspm_posture_metrics(db: AsyncSession = Depends(get_db)):
    ...
    tables_count = stats.get("tables_count", 1) or 1
